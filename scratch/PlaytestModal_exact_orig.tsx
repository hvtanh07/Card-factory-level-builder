                ...
                <g id="board-boxes-system">
                  {[...levelData.BoardNodes]
                    ...
                    .map(bn => {
                      ...
                      className={`absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all duration-200 select-none ${
                        isBlocked
                          ? 'opacity-80 brightness-75 scale-95'
                          : 'hover:scale-105 hover:brightness-110 active:scale-95 shadow-xl'
                      }`}
                      style={{
                        left: posX,
                        top: posY,
                        width: boxType.width * gridScale * 0.9,
                        height: boxType.length * gridScale * 0.9,
                        transform: `translate(-50%, -50%) rotate(${boardNode.YRotation}deg)`,
                      }}
                    >
                      {/* Box / Tray Outer Body */}
                      <div
                        className="w-full h-full rounded-2xl border-2 shadow-lg flex flex-col items-center justify-between p-1 relative overflow-hidden"
                        style={{
                          backgroundColor: isTray ? '#e2e8f0' : isRainbow ? '#38bdf8' : colorDef.hex,
                          borderColor: isBlocked ? '#475569' : '#ffffff',
                        }}
                      >
                        {/* Rainbow Shimmer */}
                        {isRainbow && !isTray && (
                          <div className="absolute inset-0 bg-gradient-to-r from-rose-500/30 via-amber-500/30 to-sky-500/30 animate-pulse pointer-events-none" />
                        )}
