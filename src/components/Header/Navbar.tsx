...
interface NavbarProps {
  ...
  onEditorModeChange: (mode: 'level' | 'conveyor') => void;
  ...
  // Conveyor specific props
  conveyorData: ConveyorData;
  conveyorName: string;
  availableConveyors: Array<{ id: string | number; name: string; data: ConveyorData }>;
  onSelectConveyor: (data: ConveyorData, name: string) => void;
  onOpenConveyorJsonModal: () => void;
  onOpenLoadConveyorJsonModal: () => void;
    ...
    <header className="h-14 bg-slate-950 border-b border-slate-800 px-4 flex items-center justify-between select-none z-30">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-sky-400 font-bold block tracking-wider">
              {editorMode === 'level' ? 'LEVEL BUILDER' : 'CONVEYOR BUILDER'}
            </span>
          </div>
        </div>
        ...
        <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800">
          <button
            onClick={() => onEditorModeChange('level')}
          ...
          <button
            onClick={() => onEditorModeChange('conveyor')}
            className={`px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1.5 transition ${
              editorMode === 'conveyor'
          ...
          >
            <Repeat size={13} />
            <span>Conveyor Editor</span>
          </button>
        </div>

        <div className="h-6 w-px bg-slate-800 mx-1"></div>

        {/* Mode-Specific Context Name & Quick Selector */}
        {editorMode === 'level' ? (
          <div className="flex items-center gap-2">
            <input
              type="text"
            ...
            {/* Paired Conveyor Selector */}
            <div className="relative flex items-center">
              <span className="text-[11px] text-slate-400 mr-1.5 hidden md:inline">Conveyor:</span>
              <select
                value={conveyorData.Id}
                onChange={(e) => {
                  const found = availableConveyors.find(c => String(c.data.Id) === e.target.value);
                  if (found) onSelectConveyor(found.data, found.name);
                }}
                className="bg-slate-900 border border-slate-700 text-sky-400 text-xs font-semibold rounded-lg px-2 py-1 pr-6 focus:outline-none focus:border-sky-500 appearance-none cursor-pointer"
              >
                {availableConveyors.map(c => (
                  <option key={`conveyor-opt-${c.id}`} value={c.data.Id} className="bg-slate-900 text-slate-200">
                    #{c.data.Id}: {c.name}
                  </option>
                ))}
              </select>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-200 bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-700">
              Conveyor #{conveyorData.Id}: <span className="text-indigo-400">{conveyorName}</span>
            </span>
            <span className="text-[11px] text-slate-400 bg-slate-900/60 px-2 py-0.5 rounded border border-slate-800">
              Level: <strong className="text-slate-300">{levelName}</strong>
            </span>
          </div>
        )}
      </div>

      {/* Center: Viewport & Canvas Toggle Tools (Level Mode) */}
      {editorMode === 'level' && (
        ...
        {editorMode === 'level' ? (
          <>
            ...
            <button
              ...
              title="View & Edit Level JSON text"
            >
          ...
          <>
            ...
            <button
              onClick={onOpenLoadConveyorJsonModal}
              className="py-1.5 px-3 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700 text-xs font-medium flex items-center gap-1.5 transition active:scale-95 shadow-sm"
              title="Input or paste Conveyor JSON text directly"
            >
              <FileJson size={14} className="text-emerald-400" />
              <span>Input JSON</span>
            </button>
            ...
            <button
              ...
              className="py-1.5 px-3 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700 text-xs font-medium flex items-center gap-1.5 transition"
              title="View, edit, or copy Conveyor JSON"
            >
            ...
            <button
              ...
              className="py-1.5 px-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold flex items-center gap-1.5 transition active:scale-95 shadow-md shadow-emerald-900/40"
              title="Export Conveyor as .json file"
            >
              <Download size={14} />
              <span>Export Conveyor</span>
            </button>
          </>
        )}

        {/* Playtest Simulator: Always visible in BOTH modes */}
        <button
          ...
          title="Playtest current level and conveyor together"
        >
